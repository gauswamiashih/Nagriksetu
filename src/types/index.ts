...
export interface IssueContextType {
  ...
  userIssues: Issue[];
  isLoading: boolean;
  createIssue: (issue: Omit<Issue, 'id' | 'createdAt' | 'updatedAt' | 'severity' | 'status' | 'category' | 'district' | 'userName'> & { category_id?: string; category?: string; district?: string; userName?: string }) => Promise<Issue>;
